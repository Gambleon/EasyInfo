...
function category(categoryObj) {
    ...
    if (!validation.hasActualValue(categoryObj)) {
        throw new Error('categoryObj must have a value');
    }
...
function item(itemObj) {
    // {id: string, name: string, content: function}
    if (!validation.hasActualValue(itemObj)) {
        throw new Error('itemObj must have a value');
    }

    var id = validation.requireHasActualValue({ obj: itemObj, property: 'id', value: property.id });
    var name = validation.requireHasActualValue({ obj: itemObj, property: 'name', value: property.name });
    var content = validation.requireHasActualValue({ obj: itemObj, property: 'content', value: property.content });
